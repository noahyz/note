什么是索引？为什么能加快查询？有什么代"最小负载算法 + 动态剔除故障节点"。负载均衡器如何维护每个 compile_server 的**实时负载**？把请求分给最小负载节点时，负载计数**分配前 +1 还是完成后统计**？动态剔除一个节点后，正在该节点上跑的请求怎么办？剔除是永久还是可恢复？什么是索引？为什么能加快查询？有什么代价？
