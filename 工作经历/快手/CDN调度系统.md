...
随着成本优化逐渐进入深水区，之前几年忽略标准化而快速上量低价资源的模式已经接近瓶颈，需要系统化的建设和精细化的运营来寻求进一步优化空间。在此背景下，需要完成看清、打通、融合三件事，完成整体大CDN的融合，实现在更大范围内寻求更优解的目标。

对于规划打通这部分，将CDN调度内部各系统进行打通，从使CDN流量能够在CDN资源自由流动，达成资源与业务抽象的归一化，为融合提供基础底座。快手近期的大目标是做NCDN。
...
- 提高调度可控带宽（之前大部分流量流离在调度体系之外，未使用调度SDK，写死、不规范接入==》域名治理）
- 提高自建/三方CDN的整体复用率
- 减少资源浪费率（包括：自建CDN的保底浪费、包端口的利用率）
- 减少回源率（主要是非直播的边缘回源）
- 基于价差考虑，提高自建CDN渗透率
- 基于质量&稳定性&效率。减小故障MTTR时长
...
PCDN和CDN打通，原因：跨省结算等政策，影响PCDN资源供给，PCDN查重失败率大幅下降。
目的：打通之后全局计算，利用PCDN对自建CDN进行填谷，保障带宽不浪费。将95机房跑到特定水位，提升自建整体利用率。
...
于是，DNS调度通过一定算法调整权重，使得每个集群的CPU和带宽不超过限制，每个节点的带宽不超过限制。 这里对于各业务的带宽和qps的预估是通过最近一小段时间在所有节点上该业务的访问日志聚合得到。 对于业务的qps在集群上会消耗多少cpu，是通过一定算法基于历史数据和压测数据来计算。对于直播、动态加速、边缘计算的消耗量，是通过他们的日志上报，当然也有一定的延迟。
