...
- 主要是为了在保证元素有序的前提下，实现高效的范围查询和插入/删除操作。代码实现简单。
- 跳表通过多级索引构建了类似”二分查找“的机制，能够快速定位起始点。非常适合 zrange、zrangebyscore等范围操作
- 查找效率都是 LogN。但是平衡树在执行范围查询时，需要复杂的树遍历，而跳表则可以通过链表高效的顺序操作。
- 插入和删除时，跳表不需要平衡树那样复杂的旋转和节点平衡操作。只需要通过随机函数生成层数并更新相应的指针即可，效率更高。
- 内存友好，相对于平衡树，跳表允许更灵活的内存分配，能够有效的利用内存空间。
